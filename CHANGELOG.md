...
- Added support for Universal Link redirect URLs with `https` scheme in Sign-In with Klarna and Klarna Identity with improved URL validation.
- Moved privacy manifest to `KlarnaCore`.
- Fixed unsynchronised mutation in internal logging which was causing a crash in certain scenarios.
- Removed internal-only types from the public API.
- Renamed and fixed incorrect types used in public API.

## [2.13.2] - 2026-08-18

- Fixed an issue in On-site Messaging (OSM) where tapping a link had no effect after cancelling the consent screen.
- Fixed a crash on iPad when presenting the sandbox browser.
...
- Separated WebView related implementations to a new `KlarnaCoreWebView` module to reduce app size for integrating apps that do not use WebView features.
- Updated payment presentation intent values.
- Updated payment and identity button configuration type and intent parameters.
- Fixed a network implementation issue for custom content-type headers impacting Sign in with Klarna integrations.

## [2.11.2] - 2026-03-26

- Fixed distribution specs missing `KlarnaNetworkIdentity` module.
- Fixed an issue with ASWebAuthenticationSession usage where a redirect would dismiss other fullscreen windows.
- Refactored the internal implementation for WebView communication to improve isolation of features.
- Added internal module subspecs to allow transitive dependencies from multiple React Native modules.
...
- Improved parameters validation in Sign in With Klarna integration.
...
- Added Klarna Messaging integration as part of Klarna Network with new `KlarnaNetworkMessaging` module.
...
- Fixed an issue where snapshot view for WebView transition was not removed after.
